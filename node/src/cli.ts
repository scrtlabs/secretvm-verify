...
function formatCheckLine(name: string, passed: boolean): string {
  // GPU is optional unless --enforce-gpu is set, so a missing GPU quote is
  // informational rather than a failure here (enforcement surfaces via gpu_present).
  if (name === "gpu_quote_fetched" && !passed) {
    return `  ${"gpu:".padEnd(35)} GPU not present`;
  }
...
// When set, require a verifiable GPU attestation for --secretvm (a CPU-only VM
// then fails closed instead of silently passing). Off by default.
...
Options:
  ...
  --enforce-gpu        Require a verifiable GPU attestation for --secretvm. A CPU-only
                       VM (no /gpu) then fails closed instead of passing.
